...
const nstateStart = Symbol();
const nstateInt = Symbol();
const nstateDec = Symbol();
const nstateExp = Symbol();
const nStateExpDigits = Symbol();
const nStateDone = Symbol();

const nctDigit = Symbol();
const nctDec = Symbol();
const nctSign = Symbol();
const nctExp = Symbol();
const nctSpace = Symbol();

/** @type {Map<string,Symbol>} */
const numberCharTypes = new Map([
  ['0', nctDigit],
  ['1', nctDigit],
  ['2', nctDigit],
  ['3', nctDigit],
...
export function getNumberOfDecimalDigits(str) {
  ...
  for (let index = str.length - 1; index >= 0; index--) {
    ...
    switch (c) {
      ...
      case 'e':
      case 'E':
        return getNumberOfDecimalDigitsOrig(str);
    }
  }
  return 0;
}
 ...
 * @param {number|string} str
 */
function xgetNumberOfDecimalDigits(str) {
  if (typeof str === 'number') {
    str = str.toString();
  }
  let state = nstateStart;
  let decimals = 0;
  let decimalZeroCount = 0;
  let integerZeroCount = 0;
  let expIntVal = 0;
  let expIsNegative = false;

  for (const c of str) {
    const charType = numberCharTypes.get(c);
    switch (state) {
      case nstateStart:
        switch (charType) {
          case nctDigit:
      ...
      case nstateExp:
        switch (charType) {
          case nctSign:
            state = nStateExpDigits;
            expIsNegative = c === '-';
            break;
          case nctDigit:
            state = nStateExpDigits;
            expIntVal = c.charCodeAt(0) - zeroCharCode;
            break;
          default:
            throw numberParseError(c, str);
        }
        break;
      case nStateExpDigits:
        switch (charType) {
          case nctDigit:
            expIntVal = 10 * expIntVal + (c.charCodeAt(0) - zeroCharCode);
            break;
          case nctSpace:
            state = nStateDone;
            break;
      ...
      case nStateDone:
        ...
        break;
      default:
        throw new Error(`unexpected state in number "${str}"`);
    }
  }

  if (decimals === 0) {
    // If there is no decimal part, trailing zeros in the integer might be significant.
    decimals = -integerZeroCount;
  }

  if (expIsNegative) {
    expIntVal = -expIntVal;
  }
  return Math.max(0, decimals - expIntVal);
}

/**
 * @param {string} str
 */
function getNumberOfDecimalDigitsOrig(str) {
  if (str.includes('e')) {
 ...
 * @returns {Error}
 */
function numberParseError(c, str) {
  return new Error(`unexpected character "${c}" in number "${str}"`);
}
